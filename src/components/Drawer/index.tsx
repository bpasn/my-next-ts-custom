...
const DrawerComponent = (props: Props) => {
    const [targetEl, setTargetEl] = React.useState<HTMLElement | null>();
    const [drawer, setDrawer] = React.useState<DrawerInterface>();
    const [options, setOptions] = React.useState<DrawerOptions>();
    React.useEffect(() => {
        setOptions({
            edge:true,
            // backdrop: true,
        ...
        setTargetEl(document.getElementById("drawer-backdrop"))
        if (targetEl) {
            setDrawer(new Drawer(targetEl, options))
        }

    }, [targetEl])

    return DrawElement(drawer)
}
...
function DrawElement(drawer?: DrawerInterface) {
    drawer?.show();
    /**
     * 
     * this Error client.js:1 Drawer with id drawer-backdrop has not been initialized. Please initialize it using the data-drawer-target attribute.
     * u can fix it, u just by using any button or element to reference to your drawer,
     */
    return <div  id="drawer-backdrop" className="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800" tabIndex={-1} aria-labelledby="drawer-backdrop-label">
        <h5 id="drawer-backdrop-label" className="text-base font-semibold text-gray-500 uppercase dark:text-gray-400">Menu</h5>
        <button type="button" onClick={() => {
            drawer?.hide()
        }} data-drawer-hide="drawer-backdrop" aria-controls="drawer-backdrop" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <ul className="space-y-2 font-medium">
                <li>
                    <a href="#" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                        <svg aria-hidden="false" className="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M2 10a8 8 0 018-8v8h8a8 8 0 11-16 0z"></path><path d="M12 2.252A8.014 8.014 0 0117.748 8H12V2.252z"></path></svg>
